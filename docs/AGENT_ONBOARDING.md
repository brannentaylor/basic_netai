...
**TIG telemetry:** Telegraf + InfluxDB + Grafana on a **dedicated** Ubuntu server **TIGger** (**not** the syslog/control VM on **`10.0.0.0/24`** — **`docs/monitoring/tig/README.md`**). **This lab, TIGger mgmt IP:** **`10.0.0.24`**. **SNMP first** (**Phase A2:** **`infra/ansible/playbooks/csr_snmp.yml`** ACL + **`install_telegraf_snmp_csr.sh`**); **gNMI deferred** (**`docs/monitoring/tig/gnmi-roadmap.md`**) until IOS upgrade — **`docs/design/2026-05-10-tigger-TIG-snmp-phased.md`**.
