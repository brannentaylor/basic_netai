...
1. From repo root: **`sudo bash infra/tig/install_telegraf_smoke.sh`** (re-run anytime; **APT does not reinstall** if already current).
   - Writes **`/etc/telegraf/telegraf.d/99-smoke-local.conf`** with **`outputs.influxdb_v2`** **`token = "${INFLUX_TOKEN}"`** (Telegraf **≥1.38** rejects **`token_file`** on that plugin). Injects the token via **`/etc/telegraf/influx_smoke.env`** + **`EnvironmentFile=`** in **`/etc/systemd/system/telegraf.service.d/basic-netai-smoke.conf`**, and sets **`TELEGRAF_OPTS=`** to silence **“unset TELEGRAF_OPTS”** warnings.
   ...
   - Debian-style units load **`*.conf`** under **`telegraf.d`**. Legacy **`@include`** footer from earlier scripts is stripped if still present.
